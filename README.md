...
1. 下载仓库（或 Releases 中的压缩包）
2. 解压后进入 `Build/` 目录
3. 在对应平台下：
   - Windows：双击 `KitchenChaos.exe`（文件名以实际为准）
4. 若运行失败，请检查：
   - 操作系统位数是否匹配（项目通常为 64 位）
   - 是否被安全软件拦截

### 4.2 在 Unity 中打开工程

1. 使用 Unity Hub 选择「Add project from disk」
2. 指向仓库根目录（或 `Project/` 子目录，视实际结构而定）
3. 用与教程相近版本的 Unity 打开（例如 2021/2022 LTS）
4. 打开主场景（例如 `Assets/Scenes/GameScene.unity`）
5. 点击 Play 即可运行
...
如果你是面试官或者对本项目感兴趣的开发者，希望深入了解：

- 我在 Unity / C# 方面的学习路径
- 或我在项目中的具体实现、思路与取舍

欢迎通过简历中提供的邮箱 / 联系方式与我沟通。
